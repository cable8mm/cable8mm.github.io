...
author: Samgu Lee
header:
  og_image: /assets/images/laravel-nova-homepage.png
---

라라벨 노바는 라라벨의 제작자인 테일러 오트웰\(Taylor Otwell\)이 직접 제작한 라라벨 기반의 어드민 대시보드입니다.

![라라벨 노바 홈페이지]({{ page.header.og_image }})

여타 어드민과 다른점은 라라벨 기반 위에 어드민 대시보드를 올린것이 아니라 패키지 형태를 취하기 때문에 현재 사용하는 웹서비스에 그대로 올릴 수 있다는 것이 다른 제품과 차별점이라고 할 수 있습니다.
